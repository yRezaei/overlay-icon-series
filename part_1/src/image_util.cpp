...
Image read_image(const std::string &path, int request_channels) {
    Image img;
...
void render_centered_text(Image &img, const char *text, const Font &font, const Color &color) {
...
Image resize_overlay(const Image &overlay, const Size &new_size) {
    Image resized_overlay;
    ...
    stbir_pixel_layout layout = (overlay.channels == 1) ? STBIR_1CHANNEL :
                                (overlay.channels == 2) ? STBIR_2CHANNEL :
...
Size calculate_new_size(const Size &base_size, const Size &overlay_size, float ratio) {
    ...
    return Size{static_cast<int>(overlay_size.width * scale), static_cast<int>(overlay_size.height * scale)};
}

std::pair<int, int> calculate_overlay_position(const Size &base_size, const Size &overlay_size, OverlayPosition position) {
    int x = 0, y = 0;
...
Image overlay_image(const Image &base, const Image &overlay, float ratio, OverlayPosition position) {
    Size new_size = calculate_new_size(base.size, overlay.size, ratio);
    Image resized_overlay = resize_overlay(overlay, new_size);
    Image result = base; // Start with a copy of the base image
...
void save_image(const Image &img, const char *file_path) {
